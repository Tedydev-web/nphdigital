...
export default function Footer3() {
	...
	const options = useMemo(
		() => ({
			fpsLimit: 30, // Giảm số FPS để tiết kiệm tài nguyên
			interactivity: {
				events: {
					onClick: {
						enable: false,
				...
				modes: {
					push: {
						quantity: 2, // Giảm số lượng hạt khi click
					},
					repulse: {
						distance: 150,
						duration: 0.2,
			...
			particles: {
				...
				links: {
					color: '#ffffff',
					distance: 200, // Tăng khoảng cách giữa các hạt để giảm số lượng kết nối
					enable: true,
					opacity: 0.4, // Giảm opacity của liên kết
					width: 0.8,
				},
				collisions: {
					enable: false,
				...
				move: {
					...
					random: false,
					speed: 0.5, // Giảm tốc độ di chuyển của hạt
					straight: false,
				},
				number: {
					density: {
						enable: true,
						area: 1000, // Tăng vùng tính toán để hạt phân tán đều hơn
					},
					value: Math.min(particleValue, 50), // Giảm số lượng hạt tối đa
				},
				opacity: {
					value: 0.4, // Giảm opacity để các hạt ít nổi bật hơn
				},
				shape: {
					type: 'circle',
				},
				size: {
					value: { min: 1, max: 2 }, // Giảm kích thước của hạt
				},
			},
			detectRetina: false, // Tắt chế độ Retina để giảm tải
			background: {
				color: 'transparent',
			},
			polygon: {
				draw: {
					enable: true,
					lineColor: 'rgba(255,255,255,0.1)', // Giảm opacity của đường viền polygon
					lineWidth: 0.3,
				},
				move: {
					radius: 10,
